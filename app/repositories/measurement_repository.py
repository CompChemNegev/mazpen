...
class MeasurementRepository(BaseRepository[Measurement]):
    ...
    async def get_by_id(
        ...
        result = await self.db.execute(select(Measurement).where(*conditions))
        return result.scalar_one_or_none()
    ...
    async def get_filtered(
        ...
        stmt = select(Measurement).offset(skip).limit(limit).order_by(sort_column)
        if conditions:
    ...
    def _build_conditions(self, filters: MeasurementFilterParams) -> list[Any]:
        ...
        if filters.measurement_type:
            conds.append(Measurement.measurement_type == filters.measurement_type)
        if filters.status:
            conds.append(Measurement.status == filters.status)
        if filters.team:
            conds.append(Measurement.team == filters.team)
        if filters.calculated_field_type:
            conds.append(Measurement.calculated_field_type == filters.calculated_field_type)
    ...
    async def get_aggregate_stats(
        ...
        stmt = (
            ...
            .group_by(Measurement.measurement_type, Measurement.unit)
        )
        conds: list[Any] = []
        conds.append(Measurement.scenario_id == scenario_id)
        if measurement_type:
            conds.append(Measurement.measurement_type == measurement_type)
        if from_date:
    ...
    async def get_map_data(
        ...
        stmt = select(
            ...
            Measurement.value,
            Measurement.unit,
            Measurement.calculated_field,
            Measurement.calculated_field_type,
        ...
        return [
            {
                ...
                "value": r.value,
                "unit": r.unit,
                "calculated_field": r.calculated_field,
                "calculated_field_type": r.calculated_field_type,
