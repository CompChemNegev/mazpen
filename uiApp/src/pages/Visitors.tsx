...
import { Search, Plus, UserPlus, Activity, Eye } from 'lucide-react';
...
export default function Visitors() {
  ...
  const filtered = visitors.filter(v => {
    ...
    if (search) {
      ...
      return v.name.toLowerCase().includes(q) || v.idNumber.toLowerCase().includes(q) || v.contact.includes(q);
    }
    return true;
  });
  ...
  const stats = {
    ...
    cleared: visitors.filter(v => v.status === 'cleared').length,
    observation: visitors.filter(v => v.status === 'under_observation').length,
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <Link
          to="/visitors/intake"
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors text-sm self-start"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
          />
        </div>
        <select
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
          className="px-3 py-2.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
        >
              ...
              {filtered.map(v => (
                <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">Age {v.age} · {v.contact}</div>
                  </td>
                  <td className="px-4 py-3 hidden sm:table-cell font-mono text-xs text-gray-600 dark:text-gray-400">{v.idNumber}</td>
                  <td className="px-4 py-3">
                    <span className={`font-mono font-semibold ${
                      v.exposureReading >= 1 ? 'text-red-500' : v.exposureReading >= 0.3 ? 'text-yellow-500' : 'text-green-500'
                    }`}>
                      {v.exposureReading.toFixed(2)} μSv/h
                    </span>
                  </td>
                  <td className="px-4 py-3"><StatusBadge value={v.status} type="visitor" /></td>
                  <td className="px-4 py-3 hidden md:table-cell text-gray-600 dark:text-gray-400">{v.movements.length} {t('visitors.locations')}</td>
                  <td className="px-4 py-3 text-right">
                    <Link
                      to={`/visitors/${v.id}`}
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-xs font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                    >
